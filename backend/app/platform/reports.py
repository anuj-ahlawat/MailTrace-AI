...
def snapshot_case(case,report,user):
    ...
    sections={
        ...
        'Risk Score':[{'email_id':a['email_id'],'score':a['risk_score'],'contributions':a['contributions'],'formula':a['formula'],'version':a.get('risk_version'),'rules':a.get('correlation_rules',[]),'model_review':a.get('model_review'),'review_recommendation':a.get('review_recommendation')} for a in analyses],
        ...
        'Geolocation':[{'email_id':a['email_id'],'assessment':a.get('geolocation'),'analyzed_hops':a.get('analyzed_hops',[]),'results':[p for i in a['intelligence'] for p in i['providers'] if p['provider']=='geoip']} for a in analyses],
